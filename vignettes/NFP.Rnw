...
\title{Network Fingerprint: a knowledge-based characterization of
biomedical networks in R}

\author{Yang Cao\footnote{yiluheihei@gmail.com}}

\begin{document}

% \sloppy  % break long line
...
computing platform \R{}, and present \CRANpkg{NFP}, an \Bioconductor{} package for fingerprint-based network analyzing and comparison of systems. Driven by the
research needs of customers, \CRANpkg{NFP} provides a unified interface to three
similarity clustering algorithms. In addition, \CRANpkg{NFP} can also provide multiscale statistical analysis and visualization, access to the specific attributes of different disease networks.

This manual is a brief introduction to structure, functions and usage of
\CRANpkg{NFP} package. The NFP package provides a set of functions to support
...
The three main features of \CRANpkg{NFP}:
...
\CRANpkg{NFP} requires these packages: \CRANpkg{magrittr}, \CRANpkg{igraph},
\CRANpkg{plyr}, \CRANpkg{ggplot2}, \CRANpkg{apcluster}, \CRANpkg{dplyr},
\CRANpkg{stringr}, \Biocpkg{graph} and \Biocpkg{KEGGgraph}. To install \CRANpkg{NFP}, please note especially two depencies of \CRANpkg{NFP}, \Biocpkg{graph} and \Biocpkg{KEGGgraph} are only available from \href{http://bioconductor.org}{Bioconductor}. Appanrantly, function \Rfunction{install.packages} can not insall Biocondutor packages. There is a function \Rfunction{biocLite}, a wrapper around \Rfunction{install.packages} provided by Bioconductor, can be used to install both CRAN and Bioconductor packages simply. More details on \Rfunction{biocLite} is available from \url{https://www.bioconductor.org/install/}. Thus, users can install NFP install the latest released version using \Rfunction{biocLite} directly:
...
It also allows users to install the latest development version from github, which requires  \CRANpkg{devtools} package has been installed on your system (or can be installed using \Rcode{install.packages("devtools")}. Note that devtools sometimes needs some extra non-R software on your system -- more specifically, an Rtools download for Windows or Xcode for OS X. There's more information about devtools \href{https://github.com/hadley/devtools}{here}.
...
After installation, the \CRANpkg{NFP} is ready to load into the current workspace
...
Moreover, gene similarity data used in our \CRANpkg{NFP} package is stored in a external data repository \Rpackage{NFPdata} \url{https://github.com/yiluheihei/datarepo} for the large size (about 16 MB). More details on how to construct External Data Repositories using the Additional\_repositories field see The Coatless Professor \href{http://thecoatlessprofessor.com/programming/r-data-packages-in-external-data-repositories-using-the-additional\_repositories-field/}{blog post}. Thus, users must install the \Rpackage{NFPdata} before the networkfinger print analyis as following code.
...
We will demonstrate go through an analysis pipeline to illustrate some of the main functions in \CRANpkg{NFP}. This pipeline consists of several steps:
...
the reference networks. We could observe the differences among biological
...
We defined a new S4 class \Rclass{NFPrefnet} to store the \CRANpkg{NFP} reference
networks. \CRANpkg{NFP} also provides five kinds of methods for this S4 class:
...
3.\Rfunction{subnet}: Extract or replace parts of the \CRANpkg{NFP} basic reference
networks.
...
\CRANpkg{NFP} provides the \Rfunction{calc\_sim\_score} function for calculating the similarity score between two networks. As the similarity score is subjected to the size of the network, we use the maslov’s method \cite{Standardization} to randomize a network while preserving the degree distribution. The \Rcode{nperm} parameter is added to
...
\CRANpkg{NFP} provides the \Rfunction{plot\_NFP\_NFP} function to visualize the network fingerprint of a single query network.
