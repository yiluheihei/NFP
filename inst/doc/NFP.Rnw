...
\CRANpkg{stringr}, \Biocpkg{graph} and \Biocpkg{KEGGgraph}. These should be installed automatically when you install {\it NFP} with \Rfunction{biocLite()} as follows:
...
\Rcode{install.packages("devtools")}. Note that devtools sometimes needs some extra
...
been developed for biological network research, e.g. KEGG, Reactome (\url{www.reactome.org}). All of this pathway databases is well-stuied and can be used as the
basic reference networks of NFP.
...
{\it NFP} provides the \Rfunction{plot\_NFP} function to visualize the network fingerprint of a single query network.
